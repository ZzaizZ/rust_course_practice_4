...
/// Отражает изображение в формате RGBA8 по горизонтали и/или вертикали.
///
/// Функция экспортируется по C ABI и вызывается хост-приложением через `libloading`.
/// Направления отражения задаются полями `flip_h` и `flip_v` в YAML-параметрах.
///
/// # Safety
///
...
pub unsafe extern "C" fn process_image(
    ...
    mirror::mirror(data, width, height, params.flip_v, params.flip_h);
}
